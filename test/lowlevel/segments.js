...
describe('segments', () => {
  ...
  describe('serialize', () => {
    it('should work without specifying the heapPos', () => {
      ...
      expect(serialize(buffer, objs)).to.equalBytes('84000000' + '00')

      objs = [std.Uint32.from(132), std.Str.from('abc'), std.Bool.FALSE]
      buffer = new Uint8Array(16)
      expect(serialize(buffer, objs)).to.equalBytes(
    ...
    it('should work specifying heapPos', () => {
      let objs = [std.Uint32.from(132), std.Bool.FALSE]
      let buffer = new Uint8Array(5)
      expect(serialize(buffer, objs, { heapPos: 5 })).to.equalBytes('84000000' + '00')

      objs = [std.Uint32.from(132), std.Str.from('abc'), std.Bool.FALSE]
      buffer = new Uint8Array(16)
      expect(serialize(buffer, objs, { heapPos: 13 })).to.equalBytes(
        '84000000' + // first object
        '0d000000' + '03000000' + // segment pointing to the string
        '00' + // boolean
        '616263' // encoded string
    ...
    it('should throw on under-allocation of memory', () => {
      ...
      expect(() => serialize(buffer, objs)).to.throw(/under-allocation/i)
    ...
    it('should throw on over-allocation of memory', () => {
      ...
      expect(() => serialize(buffer, objs)).to.throw(/over-allocation/i)
    ...
    it('should throw on incorrect specification of the heapPos', () => {
      ...
      expect(() => serialize(buffer, objs, { heapPos })).to.throw(/main segment.*does not match.*heap/i)
      heapPos = 14
      expect(() => serialize(buffer, objs, { heapPos })).to.throw(/main segment.*does not match.*heap/i)
    })
  })
